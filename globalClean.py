...
if( not data['FECHACIR'].isnull().all() and not data['FECHAFIN'].isnull().all()):
    ...
    reference_date = datetime.combine(date.today(), datetime.min.time())
...
cols_oneHot = ["OBESO", "TABACO", "TACTOR", "GLEASON1", "NCILPOS", "TNM1", "GLEASON2", "LOCALIZ", "TNM2", "PTEN", "KI_67"]

for col_name in cols_oneHot:
    data[col_name] = pd.Categorical(data[col_name], categories=rangos_cat[col_name])
    data = pd.concat([data, pd.get_dummies(data[col_name], prefix=col_name)], axis=1)
